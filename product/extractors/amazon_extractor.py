...
from product.objects.amazon_product import AmazonProduct
from rake.rake import Rake, SmartStopList
    ...
    try:
        ...
        if string_buffer is not None:
            ...
            for span in string_buffer:
                ...
                if string_buffer_3 is not None:
...
def check_page(parsed):
    ...
    if check_asin_2 is None and check_asin_3 is None:
        print("Page is Not a Product")
        return False, None
    else:
        if check_asin_2 is not None:
            asin = check_asin_2.findParent().text[5:]
        if check_asin_3 is not None:
